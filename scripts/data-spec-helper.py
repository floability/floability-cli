...
def compute_checksum(path: Path, alg: str = "sha256", chunk_size: int = 1024 * 1024) -> str:
	h = hashlib.new(alg)
...
def parse_args(argv: List[str]) -> argparse.Namespace:
	p = argparse.ArgumentParser(description="Generate Floability data spec from a directory of files")
	p.add_argument("input_dir", type=str, help="Directory containing files to include")
	p.add_argument("--output", "-o", type=str, default="-", help="Output path for YAML (default: stdout)")
	p.add_argument("--recursive", action="store_true", default=True, help="Recurse into subdirectories")

	# Source options
	p.add_argument("--source-scheme", choices=["fs", "backpack"], default="fs", help="How to express sources")
	p.add_argument("--backpack-root", type=str, help="Backpack root used to compute backpack:// relative paths")

	# Target options
	p.add_argument("--target-mode", choices=["basename", "relative", "absolute"], default="relative", help="How to set target_location (relative includes directory relative to parent of input_dir)")
	p.add_argument("--target-dir", type=str, help="Base output dir when --target-mode absolute")

	# Verification/policy
	p.add_argument("--checksum-alg", choices=["sha256", "md5", "none"], default="sha256", help="Checksum algorithm to include")
	p.add_argument("--no-expected-size", action="store_true", help="Do not include expected_size field")
	p.add_argument("--verification-type", choices=["size_only", "strict"], default="strict", help="Profile verification policy")
	p.add_argument("--size-tolerance-bytes", type=int, default=0, help="Policy size_tolerance_bytes")
	p.add_argument("--fs-absolute", action="store_true", help="Use absolute paths for fs sources (default: relative to parent of input_dir)")
	p.add_argument("--timeout", type=int, default=30, help="Policy timeout in seconds (default: 30)")
	# Duplicates
	p.add_argument("--duplicate-profiles", type=str, help="Comma-separated list of source types to duplicate profile for (e.g., pelican,http,s3)")

	# Profiles
	p.add_argument("--profile-name", type=str, default="profile-0", help="Name for generated profile")
	p.add_argument("--default-profile", type=str, help="default_profile at top-level (defaults to profile-name)")
...
def main(argv: List[str]) -> int:
	...
	spec = build_spec(
		...
		duplicate_profiles=[s.strip() for s in args.duplicate_profiles.split(",")] if args.duplicate_profiles else None,
	)
